...
export default function InstituteDashboard() {
  ...
  return (
    <div className="bg-slate-900 text-slate-300 min-h-screen flex font-sans">
      {isSidebarOpen && <div className="fixed inset-0 bg-black/60 z-40 lg:hidden" onClick={() => setIsSidebarOpen(false)}></div>}

      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-800/80 backdrop-blur-sm p-6 flex flex-col justify-between transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:bg-slate-800/50`}>
        <div>
          <div className="flex items-center justify-between mb-10">
            ...
            <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-1 text-slate-400 hover:text-white"><XIcon className="w-6 h-6" /></button>
          </div>
          <nav className="space-y-2">
            {/* ✅ These NavLinks now control the routes */}
            <NavLink to="/institute/dashboard" end className={({isActive}) => `flex items-center space-x-3 rounded-lg py-2 px-3 transition-colors ${isActive ? 'text-white bg-blue-500/20' : 'hover:bg-slate-700/50'}`}>
                <HomeIcon className="w-5 h-5" />
                <span>Dashboard</span>
            </NavLink>
            <NavLink to="/institute/dashboard/profile" className={({isActive}) => `flex items-center space-x-3 rounded-lg py-2 px-3 transition-colors ${isActive ? 'text-white bg-blue-500/20' : 'hover:bg-slate-700/50'}`}>
                <UserIcon className="w-5 h-5" />
                <span>Profile</span>
            </NavLink>
          </nav>
        </div>
        <button onClick={logoutHandler} className="w-full flex items-center justify-center space-x-2 py-2 px-3 rounded-lg bg-red-600/80 hover:bg-red-700 transition-colors text-white font-semibold">
      ...
      <main className="flex-1 p-4 sm:p-8 overflow-y-auto">
        <header className="flex items-center justify-between mb-8">
            <div className="flex items-center">
                <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden mr-4 p-1 text-slate-300"><MenuIcon className="w-6 h-6" /></button>
                <h1 className="text-xl md:text-3xl font-bold text-white">Institute Portal</h1>
            </div>
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <img src={`https://placehold.co/40x40/6366f1/ffffff?text=${instituteInfo?.name?.charAt(0) || 'I'}`} alt="User Avatar" className="w-10 h-10 rounded-full" />
                <div className="hidden sm:block">
                  <span className="text-white font-semibold">{instituteInfo?.name || 'Institute'}</span>
                </div>
              </div>
            </div>
        </header>
