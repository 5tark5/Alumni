  ...
  useEffect(() => {
    ...
    let particles = Array.from({ length: 80 }, () => {
      ...
      return new Particle(
        ...
        (Math.random() * 0.4) - 0.2, (Math.random() * 0.4) - 0.2, size
      );
    });
    const connect = () => {
            ...
            <div className="w-full max-w-sm">
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                <div>
                  ...
                  <input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500" placeholder="you@example.com" />
                </div>
                <div>
                  <label htmlFor="password" className="block text-sm mb-2 text-slate-300">Password</label>
                  <input id="password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500" placeholder="••••••••" />
                </div>
                {/* ✅ Button handles loading state */}
                <button type="submit" disabled={isLoading} className="w-full py-3 px-4 rounded-lg font-medium text-white bg-blue-600 hover:bg-blue-700 transition-all disabled:bg-blue-400 disabled:cursor-not-allowed">
              ...
              <p className="mt-8 text-center text-sm text-slate-400">
                ...
                <Link to="/institute/register" className="font-medium text-blue-500 hover:text-blue-400">
                  Register
                </Link>
              </p>
            </div>
